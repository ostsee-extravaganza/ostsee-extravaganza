...
def instagram_meta():
    """filename -> {caption, permalink} for anything tools/instagram.py pulled in."""
    p = ROOT / "data" / "instagram.json"
    if not p.exists():
        return {}
    try:
        return json.loads(p.read_text(encoding="utf-8")).get("items", {})
    except json.JSONDecodeError:
        print("    ! data/instagram.json unreadable — captions skipped")
        return {}
...
def main():
    ...
    for folder in sorted(p for p in PHOTOS.iterdir() if p.is_dir()):
        ...
        for src in originals:
            ...
            items.append({
                ...
                "permalink": meta.get("permalink") or None,
                "w": w, "h": h,
                "captured": meta.get("timestamp") or when,
                "coords": coords,
            })
    ...
    doc = {
        "note": ("Generated by tools/fotos.py — do not hand-edit the item lists. Captions on "
                 "Instagram-sourced files come from data/instagram.json and survive a rebuild; "
                 "captions typed straight in here do not. Drop files into photos/YYYY-MM-DD/ "
                 "and re-run."),
